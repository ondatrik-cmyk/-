...
# Завдання 3.
# «Хто наш найшвидший перевізник?»  Обчислити середній термін доставки для кожного перевізника (`shippers`).
# Відсортувати результати від найшвидшого до найповільнішого. Надати один графік.

shipments_with_orders = df_shipments.merge(
    df_orders[['order_id', 'ship_country']],
    on='order_id',
    how='left'
)

full_data = shipments_with_orders.merge(
    df_shippers[['shipper_id', 'name']],
    on='shipper_id',
    how='left'
)
...
shipper_report = finished_shipments.groupby('name')['delivery_duration'].mean().sort_values()

print("Середній термін доставки за перевізниками (від найшвидшого), в днях:")
print(shipper_report)

plt.figure(figsize=(10, 6))
shipper_report.plot(kind='bar', color='blue')

plt.title('Середній термін доставки за перевізником (дні)', fontsize=14)
plt.xlabel('Назва перевізника', fontsize=12)
