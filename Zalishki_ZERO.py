...
# Завдання 1.
# Виявити товари з нульовим залишком;
# використовувати дані таблиці inventory products і warehouses.

inventory_with_products = df_inventory.merge(
    df_products[['product_id', 'name']],
    on='product_id',
    how='left'
)

full_data = inventory_with_products.merge(
    df_warehouses[['warehouse_id', 'name', 'city', 'country']],
    on='warehouse_id',
    how='left'
)

zero_quantity = full_data[full_data['quantity'] == 0]

print("Перелік товарів та складів з нульовими залишками цих товарів")
print(zero_quantity)
